...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    //Overwrite theme custom menu code to add This Module item
    protected function render_custom_menu(custom_menu $menu) {
        ...
        //Copy activities block code
        $course = $this->page->course;
        require_once($CFG->dirroot.'/course/lib.php');
        $modinfo = get_fast_modinfo($course);
        $modfullnames = array();
        $archetypes = array();
        //Course == 1 means not in course pages
        if($course->id!=1) {
            ...
            foreach ($menu->get_children() as $item) {
                if ($item->get_text() == 'This Module')
                    continue;
                array_push($newChildList, $item);
            }
            $menu->override_children($newChildList);
        }
            ...
            if($item->get_text()=='This Module'){
                foreach ($modfullnames as $modname => $modfullname) {
                    if ($modname === 'resources') {
                        $item->add($modfullname, new moodle_url('/course/resources.php?id='.$course->id), $modfullname, $sort = null, $attributes = []);
                    } else {
                        $item->add($modfullname, new moodle_url('/mod/'.$modname.'/index.php?id='.$course->id), $modfullname, $sort = null, $attributes = []);
                    }
                }
            }
    ...
    public function mydashboard_admin_header() {
        ...
        $html .= html_writer::start_div('col-md-12 pb-2');
    ...
    public function user_menu($user = null, $withlinks = null) {
        ...
        if (!isloggedin()) {
            ...
            $alternativelogin = $theme->settings->alternativelogin;
            if (!empty($alternativelogin)) {
                $alternativelogintext = $theme->settings->alternativelogintext;

                if (empty($alternativelogintext)) {
                    $alternativelogintext = get_string('login');
                }

                $returnstr = "<a class='btn btn-login-top' href='{$alternativelogin}'>{$alternativelogintext}</a>";
            }

            if ($alternativelogin || !$theme->settings->disablefrontpageloginbox) {
        ...
        if (isloggedin() && !isguestuser()) {
            ...
            if ($status != 'active') {
                ...
                $content = html_writer::div($content . $dismissbtn,
                    'alert alert-danger',
                    ['style' => 'position: fixed;z-index: 9999;width: 100%;top: 70px;left: 0;right: 0;padding: 1.25rem;text-align: center;']);
