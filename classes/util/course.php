...
class course {
    ...
    public function get_summary_image_url() {
        ...
        if ($this->course instanceof stdClass) {
        ...
        return (new moodle_url('/theme/moove/pix/default_course.jpg'))->out();
    }
    ...
    public function get_category_name() {
        return core_course_category::get($this->course->category, IGNORE_MISSING)->name;
    }

    /**
     * Retorna as informacoes do formato de curso
     *
     * @return mixed
     */
    public function get_courseformat_data() {
        global $DB;

        $courseformat = course_get_format($this->course);

        if ($courseformat->get_format() != 'olm') {
            return [];
        }

        $formatoptions = $DB->get_records('course_format_options', ['courseid' => $this->course->id]);
        $formatdata = [];

        foreach ($formatoptions as $formatoption) {
            $formatdata[$formatoption->name] = $formatoption->value;
        }
